...
Status: v1 in progress — spec is [issue #1](https://github.com/arthishaxom/lesvi/issues/1); scaffold, shelf registration, the scan/index/serve spine, the browsing pages (home feed + shelf pages), agent metadata overrides (sidecar JSON + `lesvi:*` meta tags), instant client-side search (`?q=`), persisted pins (`POST /api/pin`), live index updates (native events or mtime polling, debounced), the fallback auth layer (token login + signed session cookie, Bearer, direct-loopback exemption), and the always-on systemd user service (`lesvi service`, `lesvi status`, verbosity controls) are in place.
